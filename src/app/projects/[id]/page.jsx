...
const ProjectDetails = ({ params }) => {
  ...
  //             className="inline-block md:px-2 p-1 hover:bg-button rounded border border-button text-button hover:text-white text-sm md:text-base transition-all hover:shadow-md"
  //           >
  //            <FaDownload />
  //           </button>
  ...
  return (
  <div className="w-full mt-11 px-4 py-2 pb-16 overflow-auto text-prime max-w-6xl mx-auto">

    {/* top section */}
    <div className="flex justify-between items-center flex-wrap gap-3 mb-5">
      <div className="flex items-center gap-2 flex-wrap">
        <h1 className="md:text-2xl text-lg font-bold text-button">{project.name}</h1>
        <span className={`${project.status == 'Completed' ? 'bg-done' : project.status == 'In Progress' ? 'bg-progress' : project.status == 'Pending' ? 'bg-pending' : 'bg-testing'} text-baseText rounded-full md:text-sm text-xs px-3 py-0.5`}>
          {project.status}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          title="Download Report"
          onClick={downloadReport}
          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-button text-button hover:bg-button hover:text-white text-sm transition-all"
        >
          <FaDownload /> Report
        </button>
        ...
        {project.users.map((user) => (user.pivot.is_admin && user.pivot.user_id == currentUser) ? (
          <div key={user.id} className="flex gap-2">
            <button
              onClick={() => setEdit(true)}
              className="flex items-center gap-1 px-3 py-1.5 text-sm text-button border border-button rounded-lg hover:bg-button hover:text-white transition"
            >
              <FaEdit />
            </button>
            <button
              onClick={() => setDel(true)}
              className="flex items-center gap-1 px-3 py-1.5 text-sm text-red-500 border border-red-500 rounded-lg hover:bg-red-500 hover:text-white transition"
            >
              <MdDelete />
            </button>
          </div>
    ...
    {/* overview with chart section */}
    <div className="grid md:grid-cols-3 gap-5 mb-6">
      <div className="md:col-span-2 bg-white border border-gray-200 rounded-xl shadow-sm p-5">
        <p className='text-gray-400 text-xs uppercase tracking-wide font-semibold mb-2'>Description</p>
        <p className="text-gray-700 leading-relaxed">
          {project.description || "No description available"}
        </p>
        <p className='text-gray-500 text-sm mt-4 pt-4 border-t border-gray-100'>
          Due Date: {project.due_date ? project.due_date : <span className='text-donetext'>Open</span>}
        </p>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 flex items-center justify-center">
        <HalfPieChart value={project.progress} />
      </div>
    </div>

    {/* tasks section */}
    <div className="mb-6">
      <p className='text-gray-400 text-xs uppercase tracking-wide font-semibold mb-3'>Tasks</p>
    ...
    <div className="bg-[#f1f6ff] border-t border-gray-300 text-gray-500 bottom-0 left-0 fixed w-full text-center text-sm">
      <span>Created by: </span>
      <span>{project.created_by.name} </span>
      <span>{new Date(project.created_at).toLocaleString()}</span>
    </div>
